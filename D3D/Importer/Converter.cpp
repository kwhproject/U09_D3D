...
void Converter::WriteMaterialData(wstring savePath)  //L"../../_Textures/Tank/Tank.material"
{
	...
	Xml::XMLElement* root = document->NewElement("Materials");	// <Materials> </Materials> 생성
...
string Converter::WriteTexture(string saveFolder, string file)
{
	...
	{
		string directory = Path::GetDirectoryName(String::ToString(this->file)); // _Asset
...
void Converter::ReadKeyframeData(asClip* clip, aiNode* node, vector<struct asClipNode>& aniNodeInfos)
{
	...
	for (UINT i = 0; i < clip->FrameCount; i++)
	{
		...
		if (asClipNode == nullptr)
		{
			...
			Matrix transform(node->mTransformation[0]);		// meshbone matrix
			D3DXMatrixTranspose(&transform, &transform);	// 90 회전
